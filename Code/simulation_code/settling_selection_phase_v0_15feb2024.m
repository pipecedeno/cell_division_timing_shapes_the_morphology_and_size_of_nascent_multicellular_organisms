...
This code is based on snowflake_sim_iterate_through_directory_forces.m so some parts of the code may
be redundant or unnecesary for its execution.
The idea of this code is to make the networks from the growth simulations into clusters and calculate
their volume in order to be able to perform settling selection.
...
Output file: It is going to save the id of the clusters that survived settling selection and their 
    ...
    Each value is separated by a comma and there is no headr in the file 
...
% outputs:
Surviving ids: File specified by output_file input. It contains the id of the cluster and its volume
Volumes_file: file with the following columns without a header (values are separated by commas):
    cluster_id, volume, initial_pos, final_position, distance_travelled, final_time
...
Date: 3Apr2024
-Made the code use the gyration radius instead of the convhull to get the volume and the diameter assuming that
it is a sphere.
-Changed the settling selection to instead of making a percentage of the total volume to be passaged, make it a 
number of cells, to make the bottleneck consistant
-the input of selection strenght is changed to number of cells that are going to survive


Date: 26Apr2024
-Modified the code so it receives the selection strength of the settling phase and it uses it to calculates the amount
...
output_file = parts{2};
selectionStrength = str2double(parts{3});
% num_surviving_cells=str2num(parts{3});
volumes_file=parts{4};
percentage_sampled_at_random=str2double(parts{5});
carrying_capacity=str2num(parts{6});
type_settling_selection=parts{7};

num_surviving_cells=carrying_capacity*percentage_sampled_at_random*selectionStrength;


%for testing purposes
% network_file='/Users/pipe/Desktop/test_settling_selection_sim/petite_second_5r_5m/sim_1/temp_dir/final_population_1.csv';
% output_file='/Users/pipe/Desktop/test_settling_selection_sim/petite_second_5r_5m/sim_1/temp_dir/surviving_ids_1.txt';
% selectionStrength=0.1;
% volumes_file='volumes_1.txt';
% percentage_sampled_at_random=1.0;
...
list_cluster_ids=zeros(1,num_cluster_ids);
list_diameters=zeros(1,num_cluster_ids);
list_volumes=zeros(1, num_cluster_ids);
cluster_sizes=zeros(1, num_cluster_ids);
...
for i = 1:length(unique_cluster_ids)
    ...
    [cell_list] = ELYES_SIM_fixed_daughter(diam, err_diam, aspRat, err_AR, pole_theta, theta, thetaVariance, distance_thresh, new_bud_prob, numGens, NEIGHBOR_THRESH, check_overlap, overlap_thresh, node1_vals, node2_vals);
    
    % Calculate Diameter from the cluster volume
    % Calculating cluster volume
    % Centers = [cell_list.Center];
    % Centers = Centers';
    % [T,V] = convhull(Centers); % It has happened sometimes that the convhull gives an error
    %but I am not sure if that was solved by also sorting by the transfer_added or if it still 
    %happen in rare conditions. If that happens check the parameter file to run the code in matlab
    %to better test what the problem is
    %diameter 
...
% save surviving cluster ids
%settling_results_(curr_transfer).csv

%# Creating output file
settling_selection_output = fopen(output_file, 'w');

%save ids of clusters that survived settling selection
for i = 1:length(survivors)
    survived=survivors(i);
    if survived
        fprintf(settling_selection_output, '%d,%f\n', list_cluster_ids(i), list_volumes(i));
    end
end

fclose(settling_selection_output);

%Creating volumes file
volume_output = fopen(volumes_file, 'w');
for i = 1:length(unique_cluster_ids)
    fprintf(volume_output, '%d,%.8f,%f,%f,%f,%f,%.8f\n', unique_cluster_ids(i), list_diameters(i), initial_pos(i), final_pos(i), distance_travelled(i), final_time, speedsize(i));
end
fclose(volume_output);
...
%% OLD Version using diameters and selectionStrenght as a percentage
% function [survivors, initial_pos, final_pos, distance_travelled, final_time, speedsize] = settling_selection_simulated(diameters, selectionStrength)
%     % Define physical constants and eppendorf tube size
%     massDensWater = 997;       % kg/m^3
%     dynViscoWater = .000891;   % Pa.s
%     massDensParticle = 1112.6; % kg/m^3
...
%     % Calculate volume and speed for each cluster
%     radii = diameters / 2;
%     volumes = (4/3) * pi * (radii.^3);
%     speedsize = (1/18) * ((massDensParticle - massDensWater) / dynViscoWater) * gravity * (diameters.^2);

%     % Normalize the cluster volumes
%     normalizedVolumes = volumes / sum(volumes);

%     % Sample random starting positions for each cluster
%     positions = rand(size(diameters)) * eppSize;
%     initial_pos=positions;

%     %% Comment everything below if you want to set the survival threshold as 0 (they need to reach the bottom of the tube)
%     % Define the survival threshold
%     % survival_threshold = eppSize * selectionStrength;

%     % % Confirming that the survival threshold doesn't contain the needed volume since the beginning
%     % initialSurvivingVolume = sum(normalizedVolumes(positions < survival_threshold));
%     % recalculate_threshold=false;
%     % if initialSurvivingVolume > selectionStrength
%     %     recalculate_threshold=true;
%     % end

%     % % If the initial threshold had more than the desired volume by the selectionStrength then make 
%     % % the threshold 10 times smaller until the the threshold 10 times smaller until it doesn't contain
%     % % the final volume to be transfered
%     % while recalculate_threshold
%     %     initialSurvivingVolume = sum(normalizedVolumes(positions < survival_threshold));
%     %     if initialSurvivingVolume > selectionStrength
%     %         survival_threshold = survival_threshold / 10;
%     %     else
%     %         recalculate_threshold=false;
%     %     end
%     % end
%     %% use when clusters need to reach 0
%     initialSurvivingVolume=sum(normalizedVolumes(positions == 0));
...
%         % Check the volume below the survival threshold
%         % survivingVolume = sum(normalizedVolumes(positions < survival_threshold));
%         %% If they need to reach the bottom of the tube
%         survivingVolume = sum(normalizedVolumes(positions == 0));

%         % Stop if the surviving volume is greater than the selection strength
%         if survivingVolume >= selectionStrength
%             below_threshold=false;
%         end
...
%     log_file = fopen('log.txt', 'a');
%     fprintf(log_file, 'Steps settling selection: %d\n', cont_steps);
%     fprintf(log_file, 'Initial surviving proportion: %f\n', initialSurvivingVolume);
%     fprintf(log_file, 'Final surviving proportion: %f\n', survivingVolume);
%     fprintf(log_file, 'Number of surviving clusters: %d\n', sum(survivors));
%     fprintf(log_file, 'Final time: %f sec, %f min\n', cont_steps*delta_t, cont_steps*delta_t/60);
...
function [cell_list] = ELYES_SIM_fixed_daughter(diam, err_diam, aspRat, err_AR, pole_theta, theta, thetaVariance, distance_thresh, new_bud_prob, numGens, neighbor_thresh, check_overlap, overlap_thresh, node1_vals, node2_vals)
