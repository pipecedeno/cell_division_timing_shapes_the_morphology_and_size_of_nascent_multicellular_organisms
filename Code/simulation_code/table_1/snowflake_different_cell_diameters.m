...
N               = 500; % number of clusters to generatecell_list
...
if write_to_output_file==1
    ...
    % Add file header
    fprintf(volume_size_output, 'cell_diam,sim_number,num_cells,volume_clust\n');
end
...
Ncells = zeros(length(diam_vec),1);
Scells = Ncells;
Vclusters=zeros(length(diam_vec),1);
Sclusters=zeros(length(diam_vec),1);
for a = 1:length(diam_vec)
    cells_sim = cell(N,1);
    diam = diam_vec(a);
    NEIGHBOR_THRESH = 3*diam;
    % aspRat    = AR(a);
    ...
    fprintf(['Cell Diameter tested: ', num2str(a), ' / ', num2str(length(diam_vec)), '\n']);
