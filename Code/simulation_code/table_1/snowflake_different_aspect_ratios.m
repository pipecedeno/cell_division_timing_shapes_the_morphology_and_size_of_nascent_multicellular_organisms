...
if write_to_output_file==1
    ...
    % Add file header
    fprintf(volume_size_output, 'aspect_ratio,sim_number,num_cells,volume_clust\n');
end
...
for a = 1:length(AR)
    ...
    fprintf(['Aspect Ratio tested: ', num2str(a), ' / ', num2str(length(AR)), '\n']);
    ...
    fprintf(['Stdev cluster volume: ',num2str(median(cluster_volumes)),'\n'])
    fprintf(['Stdev cluster volume: ',num2str(std(cluster_volumes)),'\n']);

    sorted_data = sort(cluster_volumes);
    lower_percentile = prctile(sorted_data, 2.5);
    upper_percentile = prctile(sorted_data, 97.5);
