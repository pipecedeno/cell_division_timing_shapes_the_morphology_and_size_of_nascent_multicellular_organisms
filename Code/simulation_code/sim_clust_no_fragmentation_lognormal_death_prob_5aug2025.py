...
parser.add_argument('-p','--death_prob',dest="death_prob",required=True) #probability of cell death after division
...
def divide_cells(cluster_population, cells_to_divide, first_div_params, second_div_params, death_prob, cont_ids, sim_number, division_step, results_table, network_files):
	...
	for mother_id in list_ids:
		...
		if(cluster_population.number_of_nodes()%5 == 0):
			...
			save_network_parameters(cluster_population, sim_number, results_table)
			
			# Save network if it's the first simulation
			save_network_if_needed(cluster_population, sim_number, division_step, network_files)
...
def save_network_parameters(network, sim_number, results_table, division_step=None):
	"""Save network diameter, mothers with undivided cells, number of nodes, max edge degree, and filamentous branches to memory"""
	...
	# Store parameters in memory table
	results_table.append([sim_number, network_diameter, mothers_with_undivided, num_nodes, max_edge_degree, num_filamentous_branches])

# function to save the graphml files of the network for each step only for the first simulation
def save_network_if_needed(network, sim_number, division_step, network_files):
	"""Store network as graphml data in memory if it's the first simulation"""
	if int(sim_number) == 1:
		if division_step is not None:
			num_nodes = network.number_of_nodes()
			filename = f'network_sim1_step{division_step}_n{num_nodes}.graphml'
		else:
			filename = f'network_sim1_final.graphml'
...
def simulate_one_cluster_growth(input_variables, first_div_params, second_div_params, death_prob):
	...
	while len(cells_to_divide) > 0 and snowflake.number_of_nodes() < max_clust_size:
		...
		cont_ids, cells_to_divide = divide_cells(snowflake, cells_to_divide, first_div_params, second_div_params, death_prob, cont_ids, sim_number, division_step, results_table, network_files)
	...
	return success, results_table, network_files
...
def write_results_to_disk(results_table, network_files, dict_files, input_variables):
	"""Write stored results to disk files"""
	# Write network information to CSV
	for row in results_table:
		dict_files['diameter'].write(f"{row[0]},{row[1]},{row[2]},{row[3]},{row[4]},{row[5]}\n")
...
log_file.write("sim_clust_no_fragmentation_lognormal_death_prob_5aug2025.py\nInputs received:\n")
...
for i in range(1, num_iterations + 1):
	...
	attempt = 1
	while True:
		success, results_table, network_files = simulate_one_cluster_growth(input_variables, first_div_params, second_div_params, death_prob)
		if success:
			# Write results to disk only if simulation was successful
			write_results_to_disk(results_table, network_files, dict_files, input_variables)
